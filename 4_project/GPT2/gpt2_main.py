...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    f = open("../results/gpt2/{}/{}_res.txt".format(args.rq,args.project), "a")
    for key in sorted(result.keys()):
...
def write_raw_preds_csv(args, y_preds):
    ...
    df.to_csv("../results/gpt2/{}/{}_raw_preds.csv".format(args.rq,args.project), index=False)
...
def main():
    ...
    # Set seed
    set_seed(args)
    config = GPT2Config.from_pretrained("/mnt/sdb/zhaoyuan/zhangquanjun/ybw/models/codegpt")
    config.pad_token_id = 50257
    tokenizer = GPT2Tokenizer.from_pretrained('/mnt/sdb/zhaoyuan/zhangquanjun/ybw/models/codegpt')
    tokenizer.add_tokens(["<pad>"])
    tokenizer.pad_token = "<pad>"
    model = GPT2ForSequenceClassification.from_pretrained("/mnt/sdb/zhaoyuan/zhangquanjun/ybw/models/codegpt", config=config, ignore_mismatched_sizes=True)
