...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    f = open("../results/codebert/{}/{}_res.txt".format(args.rq,args.project), "a")
    for key in sorted(result.keys()):
...
def write_raw_preds_csv(args, y_preds):
    ...
    df.to_csv("../results/codebert/{}/{}_raw_preds.csv".format(args.rq,args.project), index=False)
...
def main():
    ...
    # Set seed
    set_seed(args)
    config = RobertaConfig.from_pretrained("/mnt/sdb/zhaoyuan/zhangquanjun/ybw/models/codebert")
    config.num_labels = 1
    config.num_attention_heads = args.num_attention_heads
    tokenizer = RobertaTokenizer.from_pretrained("/mnt/sdb/zhaoyuan/zhangquanjun/ybw/models/codebert")
    model = RobertaForSequenceClassification.from_pretrained("/mnt/sdb/zhaoyuan/zhangquanjun/ybw/models/codebert", config=config, ignore_mismatched_sizes=True)
