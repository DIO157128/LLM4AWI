...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type="train"):
        ...
        if file_type == "train":
            funcs = df["warning_line"].tolist()[:int(len(df["warning_line"].tolist())*args.fine_tune_factor)]
            labels = df["final_label"].tolist()[:int(len(df["final_label"].tolist())*args.fine_tune_factor)]
            print(len(funcs))
        else:
...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    f = open("../results/graphcodebert/{}_{}_res.txt".format(args.project,args.fine_tune_factor), "a")
    for key in sorted(result.keys()):
...
def write_raw_preds_csv(args, y_preds):
    ...
    df.to_csv("../results/graphcodebert/{}_{}_raw_preds.csv".format(args.project,args.fine_tune_factor), index=False)
